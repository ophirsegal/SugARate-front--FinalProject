...
export class NutritionService {
  ...
  async getNutritionInfo(foodQuery: string, icrRatio?: number): Promise<NutritionResponse> {
    try {
      const response = await axios.post(
        `${this.baseURL}/nutrition-info`,
        { 
          query: foodQuery,
          icrRatio: icrRatio // Include ICR ratio in request
        },
        { headers: this.getHeaders() }
      );
      return response.data;
    } catch (error) {
      console.error('[NutritionService] Error getting nutrition info:', error);
      throw error;
    }
  }
